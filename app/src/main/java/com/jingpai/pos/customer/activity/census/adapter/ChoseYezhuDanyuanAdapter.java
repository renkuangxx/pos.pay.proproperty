 ...
 * @author 86173
 */
public class ChoseYezhuDanyuanAdapter extends BaseAdapter implements Filterable {

    private List<XiaoquDanyuanInfoBean> list = new ArrayList<>();
    private Context context;
    private MyFilter filter = null;// 创建MyFilter对象
    private FilterYezhuListener listener = null;// 接口对象

    public ChoseYezhuDanyuanAdapter(List<XiaoquDanyuanInfoBean> list, Context context, FilterYezhuListener filterListener) {
        this.list = list;
    ...
    class MyFilter extends Filter {
        ...
        // 创建集合保存原始数据
        private List<XiaoquDanyuanInfoBean> original = new ArrayList<>();

        public MyFilter(List<XiaoquDanyuanInfoBean> list) {
            this.original = list;
        }
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if(TextUtils.isEmpty(constraint)){
                ...
                // 创建集合保存过滤后的数据
                List<XiaoquDanyuanInfoBean> mList = new ArrayList<>();
                // 遍历原始数据集合，根据搜索的规则过滤数据
                    for (int i = 0;i<original.size();i++){
                    // 这里就是过滤规则的具体实现【规则有很多，大家可以自己决定怎么实现】
                    if(original.get(i).getName().trim().toLowerCase().contains(constraint.toString().trim().toLowerCase())){
                        // 规则匹配的话就往集合中添加该数据
                        mList.add(new XiaoquDanyuanInfoBean(original.get(i).getBuildingId()
                                ,original.get(i).getCommunityId()
                                ,original.get(i).getName()
        ...
        protected void publishResults(CharSequence constraint,
            ...
            // 获取过滤后的数据
            list = (List<XiaoquDanyuanInfoBean>) results.values;
            ...
            if(listener != null){
                ...
                listener.getFilterDatadanyuan(list1,list);
            }
            // 刷新数据源显示
