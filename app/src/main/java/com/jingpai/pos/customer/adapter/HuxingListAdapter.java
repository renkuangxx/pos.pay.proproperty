...
//更多 列表
public class HuxingListAdapter extends RecyclerView.Adapter<HuxingListAdapter.ItemViewHolder> {
    private Context mContext;
    private List<HuxingListBean> listBeans;
    ...
    public HuxingListAdapter(Context mContext) {
        this.mContext = mContext;
    }

    public HuxingListAdapter(Context mContext, List<HuxingListBean> listBeans1
    ...
    @NonNull
    @Override
    public HuxingListAdapter.ItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_list_life, parent, false);
        HuxingListAdapter.ItemViewHolder holder = new HuxingListAdapter.ItemViewHolder(view);
        return holder;
    }
...
//        likec	初始点赞数
        String likeCount = TextUtils.isEmpty(listBeans.get(position).getLikeCount() + "") ? ""
                : listBeans.get(position).getInitThumb() + "";
        holder.tvPicTitle.setText(likeCount+"人喜欢");

//        budget	预算
        String budget = TextUtils.isEmpty(listBeans.get(position).getSquareCost()) ? "面议"
...
//        title	标题
        ...
        holder.tvPicTitle.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                setLike(isLike, baseRequest, holder);
            }
        });
    ...
    private void setLike(boolean isLike, TreeMap<String, Object> baseRequest, @NonNull ItemViewHolder holder) {
