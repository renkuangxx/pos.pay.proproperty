...
import static com.jingpai.pos.BuildConfig.LIFE_MATER_URL;


//更多 列表
public class LifeSearchAdapter extends RecyclerView.Adapter<LifeSearchAdapter.ItemViewHolder> {
    private Context mContext;
    private List<SearchBean.ListBean> listBeans;
    LifePresenter lifePresenter;
    private UpdataInterface updataInterface;

    public LifeSearchAdapter(Context mContext) {
        this.mContext = mContext;
    }

    public LifeSearchAdapter(Context mContext, List<SearchBean.ListBean> listBeans1, LifePresenter lifePresenter, UpdataInterface updataInterface) {
        this.mContext = mContext;
    ...
    @NonNull
    @Override
    public LifeSearchAdapter.ItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_list_life, parent, false);
        LifeSearchAdapter.ItemViewHolder holder = new LifeSearchAdapter.ItemViewHolder(view);
        return holder;
    }
...
//        cover	封面图片
        String pic = TextUtils.isEmpty(listBeans.get(position).getImage()) ? ""
                : listBeans.get(position).getImage();
...
//        budget	预算
        int budget = listBeans.get(position).getSquareCost();
        holder.tvPrice.setText(budget == 0?"面议" : "￥" + budget);
...
//        title	标题
        ...
        int searchType = listBeans.get(position).getSearchType();
...
//        内容或素材的搜索的类型（1：内容、2：素材）
        if (listBeans.get(position).getSearchType() == 1) {
    ...
    private void jump2Web(int searchType, String id) {
        Intent intent = new Intent(mContext, CityLifeWebViewActivity.class);
        String url = (searchType == 1 ? LIFE_CONTENT_URL : LIFE_MATER_URL)
                + "/" + id;
    ...
    public class ItemViewHolder extends RecyclerView.ViewHolder {
        ...
        private RelativeLayout rl, rl_like,rl_outside;
